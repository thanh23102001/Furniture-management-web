    ...
    <body>
       <header>
        ...
        <div class="container-fluid">
            <div class="row">
                <div style="text-align:center;" border="2px" class="col-lg-8">
                    <h2 style="margin-left: 530px" class="divider-style"><span>Danh sách yêu cầu</span></h2>
                </div>
            </div>
            <div>
                <div class="row row-space">
                    <div style="margin-left: 280px" class="col-lg-8">
                        <table class="table table-bordered" >
                            <thead>
                                <tr style="background-color: #002046; color: whitesmoke">
                                    <td><h4 style="font-family:Tim New Rone;"><b>Mã ID</b></h4></td>
                                    <td><h4 style="font-family:Tim New Rone;"><b>Tổ</b></h4></td>
                                    <td><h4 style="font-family:Tim New Rone;"><b>Nội dung</b></h4></td>
                                    <td><h4 style="font-family:Tim New Rone;"><b>Trạng thái</b></h4></td>
                                    <td colspan="2"><h4 style="font-family:Tim New Rone; text-align: center"><b>Xét duyệt</b></h4></td>
                                </tr>
                            </thead>
                            <tbody>
                                <c:forEach items="${request}" var="rq">
                                    <tr>
                                        <td>${rq.id}</td>
                                        <td>${rq.id_group}</td>
                                        <td>${rq.content}</td>
                                        <td><c:if test="${rq.status eq 0}">Đang chờ</c:if>
                                            <c:if test="${rq.status eq 1}">Từ chối</c:if>
                                            <c:if test="${rq.status eq 2}">Đã chấp nhận</c:if></td>
                                            <c:if test="${rq.status eq 0}">
                                            <td><a href="confirm?id_request=${rq.id}&status=2">Chấp nhận</a></td>
                                            <td><a href="confirm?id_request=${rq.id}&status=1">Từ chối</a></td></c:if>
