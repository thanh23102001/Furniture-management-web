...
<html>
    <head>
        ...
        <title>Danh sách thiết bị bán</title>
        ...
        <div class="container-fluid">
            <div class="row">
                <div style="text-align:center;" border="2px" class="col-lg-10">
                    <h2 class="divider-style"><span>Danh sách thiết bị bán</span></h2>
                </div>
            </div>
            <div>
                <a href="insertbillsell" role="button" class="btn btn-danger" style="margin-left: 17%;width: 180px;">Thêm hóa đơn</a>
            </div>
            <div>
                <div class="row row-space">
                    <div class="col-lg-2">
                        <form action="listsell" method="GET" id="byid">
                            <h4>Tìm kiếm</h4>
                            ...
                            Tên người mua
                            <br>
                            <select name="buyer_id" style="width: 220px">
                                <option value="0">Toàn bộ người mua</option>
                                <c:forEach items="${requestScope.buyer}" var="by">
                                    <option value="${by.id}" ${byid == by.id ? 'selected' : ""}> 
                                        ${by.name}
                                    </option>
                                </c:forEach>
                            </select>
                        ...
                        <table class="table table-bordered" >
                            <thead>
                                <tr style="background-color: #002046; color: whitesmoke">
                                    ...
                                    <td><h4 style="font-family:Tim New Rone;"><b>Đơn vị</b></h4></td>
                                    <td><h4 style="font-family:Tim New Rone;"><b>Ngày bán</b></h4></td>
                                    <td><h4 style="font-family:Tim New Rone;"><b>Người mua</b></h4></td>
                                    <td><h4 style="font-family:Tim New Rone;"><b>Xem chi tiết</b></h4></td>
                                </tr>
                            </thead>
                            <tbody>
                                <c:set var="total" value="${0}"/>
                                <c:forEach items="${sell}" var="s">
                                    <tr >
                                        <td>${s.seri}</td>
                                        <td>${s.name}</td>
                                        <td>${s.model}</td>
                                        <td>${s.price} vnd</td>
                                        <c:if test="${s.unit eq true}"><td>Bộ</td></c:if>
                                        <c:if test="${s.unit eq false}"><td>Chiếc</td></c:if>
                                        <c:set var="total" value="${total + s.price}" />
                                        <td>${s.billsell.datesell}</td>
                                        <td>${s.billsell.buyer.name}</td>
                                        <td><a href="updatesell?seri=${s.seri}&action=1" value="${s.seri}">Xem và sửa</a></td>
                                    </tr>
                                </c:forEach>
                            </tbody>
                            <tfoot>
