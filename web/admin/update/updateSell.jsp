...
<html>
    ...
    <body>
        ...
        <div class="container-fluid">
            <div>
                <form action="updatesell?direct=${action}" method="POST">
                    <div class="container ">
                            ...
                            <div class="col-md-4 offset-md-4">
                                ...
                                <select class="form-control" name="bill_id">
                                    <c:forEach items="${bill}" var="bb">
                                        <option 
                                            <c:if test="${machinesell.billsell_id eq bb.id}">
                                ...
                                Mã Seri
                                <br>
                                <input disabled="disabled" class="form-control" type="text" placeholder="Mã số khác nhau được in trên mỗi thiết bị" name="seri" value="${machinesell.seri}"/>
                                <br>
                                Đơn vị
                                <br>
                                <select class="form-control" name="unit">
                                    <option 
                                        <c:if test="${machinesell.unit eq false}">
                                            selected="selected" </c:if> 
                                            value="false">Chiếc
                                        </option>
                                        <option 
                                        <c:if test="${machinesell.unit eq true}">
                                            selected="selected" </c:if> 
                                            value="true">Bộ
                                        </option>
                                    </select>
                                    ...
                                    Giá mua
                                    <br>
                                    <input class="form-control" type="number"  name="price" value="${machinesell.price}"/>
                                <br>
                                Độ mới
                                <br>
                                <input class="form-control" type="number" min="1" max="100" placeholder="Từ 1 đến 100" name="newness" value="${machinesell.newness}"/>
                                <br>
                                <div style="margin-bottom: 10px">
                                    <input  class="btn btn-success" type="submit" value="Lưu thay đổi"/>
                                    
                                    
                                    <a href="deletesell?seri=${machinesell.seri}&direct=${action}&bill_id=${bill_id}" class="btn btn-danger">Xóa</a>
                                    
                                    <c:if test="${action eq 1}">
                                    <a href="listsell" class="btn btn-danger">Hủy</a>
                              </c:if>
                                    
                                       <c:if test="${action eq 2}">
                                    <a href="listbillsell/listsell?bill_id=${bill_id}" class="btn btn-danger">Hủy</a>
                              </c:if>
                                </div>
                            </div>
                        </div>
