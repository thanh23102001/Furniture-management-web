...
<html>
    ...
    <body>
       <header>
        ...
        <form action="updatebillsell" method="POST">
            <div class="container ">
                <div class="row row-space">
                    <div class="col-md-4 offset-md-4">
                        ...
                        Mã hóa đơn
                        <br>
                        <input class="form-control" type="text" name="id_bill" value="${billsell.id}" disabled="disabled" />
                        <br>
                        Ngày bán
                        <br>
                        <input class="form-control" type="date" name="date_sell" max="${datemax}" value="${billsell.datesell}">
                        <br>
                        Số tiền chưa nhận được
                        <br>
                        <input class="form-control" type="number" name="amount_owed" value="${billsell.amountowed}"/>
                        <br>
                        Người mua
                        <br>
                        <select class="form-control" name="buyer_id">
                            <c:forEach items="${buyer}" var="be">
                                <option value="${be.id}" <c:if test="${be.id eq billsell.buyerid}">selected="selected"</c:if>>
                                    ${be.name}
                                </option>
                            </c:forEach>
                        </select>
                        <p style="opacity: 70%">Nếu chưa có người mua tương ứng, vào mục người mua để thêm</p>
                        <div>
                            <input class="btn btn-success" type="submit" value="Sửa hóa đơn"/>
                        </div>
                    </div>
                </div>
            </div>
